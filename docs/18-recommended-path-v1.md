...
The next repo action is not to polish `0118`, `0120`, or another package. Select the next Track B source arm or controlled batch under decisions `0122` through `0131`: locked caps, no overflow queue, daily stop-rule checks, strict validation, raw-evidence exclusion before git-facing output, and aggregate-only reporting.
