# Batch 0008 Reviewer Packet

## Purpose

This packet turns Batch 0008 into a capped reviewer-hour value test.

The goal is not to decide legal fraud. The goal is to compare four reviewed
metadata lanes by high-value candidate discovery per unit reviewer effort.

Use this packet together with:

- `data/candidate_intake/batch_0008_intake.yaml`
- `data/candidate_stubs/batch_0008.yaml`
...
Batch 0008 answers one operating-system question:

```text
Which routing lane deserves scarce reviewer time after Batch 0007 context gates?
```

This batch is capped at 16 intake entries because the purpose is reviewer-hour
comparison, not collection scale.
...
`needs_thread` expected absent. This is the positive control.
...
`INTAKE_0008_A_01` through `INTAKE_0008_A_04`:
Confirm whether strong guarantee/certainty plus executable transition remains
high-value and low burden.

`INTAKE_0008_B_01` through `INTAKE_0008_B_04`:
Test whether low-context result-display transition stays cheap enough for
single-item triage.

`INTAKE_0008_C_01` through `INTAKE_0008_C_04`:
Test whether thread-required result display justifies full-thread review cost.

`INTAKE_0008_D_01` through `INTAKE_0008_D_04`:
Test hard-negative stability and protect against over-generalizing result
display.
...
- all 16 intake entries have `fill_status: completed`
