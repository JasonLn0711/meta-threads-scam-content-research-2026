...
\newcommand{\repoartifact}[1]{\path{#1}}
...
\begin{tabularx}{0.68\textwidth}{@{}Yr@{}}
\toprule
Content shape & Count \\
\midrule
...
Kept or moved to \code{insufficient_evidence}/\code{medium} &
\texttt{0073}, \texttt{0074} \\
Kept \code{insufficient_evidence}/\code{low} &
...
\begin{longtable}{@{}L{0.27\textwidth}L{0.37\textwidth}L{0.26\textwidth}@{}}
...
The baseline triage support differs slightly from the aggregate risk table because the evaluator uses \code{final\_risk\_level} when present, otherwise \code{risk\_level}.
One duplicate/insufficient-evidence trace keeps \code{risk\_level = medium} for schema history but has \code{final\_risk\_level = low} after adjudication.
Therefore the baseline triage support is \code{high 36}, \code{medium 10}, and \code{low 32}, while the checkpoint aggregate risk distribution remains \code{high 36}, \code{medium 11}, and \code{low 31}.
...
\section{Reviewer Adoption Decision Points}

Reviewers should decide whether the package is accepted for checkpoint use, whether any repo-safe edits are required, and whether broader sharing requires legal/privacy review.

\begin{table}[h]
\centering
\caption{Reviewer adoption response table.}
\begin{tabularx}{\textwidth}{@{}L{0.27\textwidth}Y@{}}
\toprule
Field & Response \\
\midrule
Checkpoint reviewed & \checkpoint{} \\
Status & \code{accepted_for_checkpoint_use} / \code{accepted_with_conditions} / \code{revision_requested} / \code{new_capped_evidence_decision_requested} / \code{blocked_until_legal_privacy} \\
...
Broader external sharing requested? & \texttt{no}, unless legal/privacy status is requested \\
Notes & \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{longtable}{@{}L{0.68\textwidth}L{0.22\textwidth}@{}}
\toprule
Artifact & Purpose \\
\midrule
\endfirsthead
