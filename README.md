...
the default drift toward more reviewer-only batches, checks official Threads API
access first, and allows controlled browser run reuse only as a new run-scoped
fallback. It explicitly rejects broad personal-account browser crawling and
one-second automated fetching as standing governance permissions.
