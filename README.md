...
The immediate next step is recipient adoption dispatch for checkpoint 0081: send the repo-safe request form through an approved internal/CIB or technical/governance channel outside git, record sent/received status in the dispatch log, then record the returned adoption response in the adoption tracker. Legal/privacy gating remains required before broader external sharing. Confirmed pointers remain the preferred future source for final scam/high-risk rule learning if a later decision requests more evidence. Browser-session capture should continue only if stakeholders explicitly approve another bounded tranche for false-positive and uncertainty calibration, with controlled capture, redacted records, fast different-angle second review, strict validation, and checkpoint synthesis. Query terms and crawler seeds remain candidate-finding tools only, not labels.
