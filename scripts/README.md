...
record_version_update.py      Update VERSION, prepend CHANGELOG.md, and append versioning/version_log.csv
```
...
For repo operating-version updates, use system `python3` or the active virtual
environment:

```bash
python3 scripts/record_version_update.py --dry-run --bump patch \
  --summary "Dry run version bump"
```
...
Record a repo operating-version update after governed changes:

```bash
python3 scripts/record_version_update.py --bump minor \
  --summary "Add governed access route" \
  --category governance \
  --category data-access \
  --path docs/51-meta-content-library-api-access.md \
  --detail "Recorded a repo-safe official access route update." \
  --verification "git diff --check"
```

This script writes repo-safe metadata only. Do not include raw Threads evidence,
source URLs, handles, screenshots, credentials, browser artifacts, access
emails, or controlled run details in version log fields.
