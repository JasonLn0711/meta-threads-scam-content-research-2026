...
| 2026-05-21 | Meta Content Library / API as preferred official route for CIB Threads research | `approved_with_limits` | Use Content Library UI/API first where available; Threads API keyword search is supplementary only if run-scoped. |
...
| MCL-THREADS-CIB-ROUTE-2026-05-21 | 2026-05-21 | Meta Content Library UI/API, with official Threads API as scoped supplement | official Meta research route; Threads API keyword search only if approved | public Threads fields exposed by approved official route, minimized text/OCR/reply/link metadata, route/query metadata, and aggregate metrics under run records | controlled cleanroom/raw export path outside git | pilot-only unless later expansion work order extends it | `approved_with_limits` | CIB stakeholder / project owner | Required preferred official route for this case; record Content Library UI/API/Threads API coverage and export limits per run. |
