...
| `GR-025` | Prior named stock wins plus limit-up/percentage proof, screenshot-style proof, luxury-result claims, and no-fee/financial-freedom sharing language form a high-risk trust funnel when confirmed by CIB/stakeholder context. | `docs/06-annotation-guideline-v1.md`, `docs/04-taxonomy.md`, `docs/30-annotator-onboarding-quickstart.md` | no | This revision reuses existing signal tags: `past_performance_profit_proof`, `testimonial_or_earnings_screenshot`, `stock_pick_playbook_keyword_funnel`, and `lifestyle_trust_market_reassurance_funnel`. Full raw URL, handle, stock names, stock codes, price values, raw post text, screenshot, HTML, and replies remain outside git. |
| `GR-026` | Humble/non-analyst/no-benefit top-level investment framing plus reply-level anti-scam camouflage that publishes a "safe" contact/group path should be treated as a reply contact-hijack/private-group funnel. | `docs/06-annotation-guideline-v1.md` | no | This revision reuses existing signal tags: `reply_impersonation_contact_hijack`, `private_channel_redirect`, `contact_handle_visible`, `stock_rescue_group_funnel`, `past_performance_profit_proof`, and `lifestyle_trust_market_reassurance_funnel`. Full raw URL, handles, contact IDs, raw post text, raw replies, screenshot, and HTML remain outside git. |
| `GR-027` | Investment-experience or trading-psychology posts with a redacted external-link category can be trust-building starters, but remain `uncertain` unless fuller context shows private-message, community, stock-tip, reassurance, guarantee, contact, or conversion mechanics. | `docs/06-annotation-guideline-v1.md` | no | This revision reuses existing signal tag `visible_external_link`; no new taxonomy/schema value is added. Full raw URL, handle, raw post text, raw replies, screenshot, and HTML remain outside git. |
