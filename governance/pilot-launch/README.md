...
Run 0009 then selected one item 0016 candidate and built a strict-valid local record under ignored `data/interim/`. Second review changed item 0016 from preliminary `uncertain` / `medium` to final `non_scam` / `low`; it is now treated as a false-positive pressure example. Run 0010 is opened for a limited item 0017-0020 extension, with candidate screening updated to exclude negated guaranteed-profit or risk-warning statements before selection. The first two run 0010 item 0017 attempts using `RP-0010-01` and `RP-0010-02` each reviewed 5 candidates and selected no item; no `manual_entry_0017.json` was created.
